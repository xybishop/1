...
const SEASON_LABELS = {
  1: { label: 'Saison 1', count: S1.length },
  2: { label: 'Saison 2', count: S2.length },
  3: { label: 'Saison 3', count: S3.length },
};
...
document.addEventListener('DOMContentLoaded', () => {
  buildSeasonTabs();
  renderGrid(1);
  document.getElementById('closeYokaiPlayer')?.addEventListener('click', closePlayer);
});
...
function openPlayer(season, ep, url) {
  const player = document.getElementById('yokaiPlayer');
  const wrap   = document.getElementById('yokaiFrameWrap');
  if (!player || !wrap) return;

  document.getElementById('yokaiEpBadge').textContent = `S${season} · Ép. ${ep}`;
  document.getElementById('yokaiEpTitle').textContent  = `Épisode ${ep}`;
  ...
  // Pas de sandbox ni referrerpolicy — Sibnet les refuse
  iframe.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;border:none;';
  wrap.appendChild(iframe);

  buildSourceBadge(url);
  player.classList.remove('hidden');
  player.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function buildSourceBadge(url) {
  const el = document.getElementById('yokaiSources');
  if (!el) return;
  let src = 'Sibnet';
  if (url.includes('sendvid'))      src = 'Sendvid';
  else if (url.includes('vidmoly')) src = 'Vidmoly';
  else if (url.includes('anime-sama')) src = 'Anime-Sama';
  else if (url.includes('dailymotion')) src = 'Dailymotion';

  el.innerHTML = `
    <div class="yw-source-badge">
      <div class="yw-source-badge-inner">
        <div>
          <p class="yw-source-label">Source actuelle</p>
          <p class="yw-source-name">${src}</p>
          <p class="yw-source-lang">VF • Auto</p>
        </div>
        <svg class="yw-check" fill="currentColor" viewBox="0 0 20 20" width="20" height="20">
...
function closePlayer() {
  ...
  currentEp = null;
  document.querySelectorAll('.yw-ep-card').forEach(c => c.classList.remove('active'));
}

function openFilm() {
  window.open(FILM_URL, '_blank', 'noopener');
}
