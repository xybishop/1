/* ══════════════════════════════════════════════════════════════
   ...
   • Son alternatif 25% : assets/sounds/death_alt.mp3
     (mets le fichier dans assets/sounds/ — si absent, son normal)
...
(function () {
  ...
  function createSectionBg() {
    ...
    wrap.style.cssText = 'position:absolute;top:15%;left:0;right:0;bottom:0;z-index:0;overflow:hidden;pointer-events:none;';
  ...
  /* Son death — 25% chance d'utiliser death_alt.mp3 */
  function playDeathSound() {
    const useAlt = Math.random() < 0.25;
    const src = useAlt ? 'assets/sounds/death_alt.mp3' : 'assets/sounds/death.mp3';
    try {
      const a = new Audio(src);
      a.volume = 0.4;
      a.play().catch(() => {
        // Si alt introuvable, fallback sur le son principal
        if (useAlt) {
          const b = new Audio('assets/sounds/death.mp3');
          b.volume = 0.4; b.play().catch(() => {});
        }
      });
    } catch(e) {}
  }
  ...
  function loop() {
    ...
    obstacles.forEach(o => {
      ...
      if (!dead && hitsObs(o)) { dead=true; spawnParticles(PL.x+PLAYER_W/2,PL.y+PLAYER_H/2); }
    });
    ...
    birds.forEach(b => {
      ...
      drawBird(b);
      if(!dead&&hitsBird(b)){dead=true;spawnParticles(PL.x+PLAYER_W/2,PL.y+PLAYER_H/2);}
    });
