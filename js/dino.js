/* ══════════════════════════════════════════════════════════════
   ...
   • BOOST : tous les 200 pts une charge se stocke sur la barre bois
     Appuyer sur M en jeu active 5s d'invincibilité + skin swap
...
(function () {
  ...
  /* ══════════════════════════════════════════
     SOL EN BOIS — texture dessinée sur offscreen
     On utilise repeat-x pour ne jamais être vide
  ══════════════════════════════════════════ */
  const WOOD_H = H - GROUND_Y;  // = 55px
  const woodCanvas = document.createElement('canvas');
  woodCanvas.width = 240;       // tile plus petite pour bien se répéter
  woodCanvas.height = WOOD_H;
  ...
  // Ombre en haut du sol (liseré sombre)
  wctx.fillStyle = 'rgba(50,28,8,0.30)';
  wctx.fillRect(0, 0, 240, 2.5);

  // ⚠️ On crée le pattern APRÈS avoir tout dessiné sur le woodCanvas
  ...
  /* ── Boost system ── */
  let boostCharges    = 0;   // charges stockées (1 par tranche de 200 pts)
  let lastBoostMile   = 0;   // dernier multiple de 200 accordé
  let boostActive     = false;
  let boostTimer      = 0;   // frames restantes (5s × 60fps = 300)
  const BOOST_DURATION = 300; // frames

  /* ── Player ── */
  const PL = { x: 100, y: GROUND_Y - PLAYER_H, vy: 0, onGround: true, angle: 0 };

  function jump() {
    if (PL.onGround) { PL.vy = JUMP_V; PL.onGround = false; }
  }
  ...
  function createVideoEl(src, muted, loop) {
    const v = document.createElement('video');
    v.src = src; v.muted = muted; v.loop = loop; v.playsInline = true;
    v.autoplay = true; v.setAttribute('playsinline', '');
    v.currentTime = 85;
    v.addEventListener('loadedmetadata', () => { v.currentTime = 85; });
    v.style.cssText = [
  ...
  function createSectionBg() {
    document.getElementById('sectionBgWrap')?.remove();
    ...
    sectionBgVideo = createVideoEl('assets/videos/bg_section.mp4', true, true);
    sectionBgVideo.volume = 0;
    wrap.appendChild(sectionBgVideo);
    sec.prepend(wrap);
    sectionBgVideo.addEventListener('canplay', () => {
  ...
  function removeSectionBg() {
    document.getElementById('sectionBgWrap')?.remove();
    sectionBgVideo = null;
  }

  function createGameBg() {
    document.getElementById('gameBgWrap')?.remove();
    ...
    gameBgVideo = createVideoEl('assets/videos/bg_game.mp4', false, true);
    gameBgVideo.volume = 0.5;
    wrap.appendChild(gameBgVideo);
    gw.prepend(wrap);
    gameBgVideo.play().catch(() => {});
    gameBgVideo.addEventListener('canplay', () => {
      if (sectionBgVideo) sectionBgVideo.currentTime = gameBgVideo.currentTime;
    }, { once: true });
  }
  ...
  /* ══════════════════════════════════════════
     MOGGED — zoom-in + fade, puis fade-out
  ══════════════════════════════════════════ */
  ...
  /* Son death — 65% death.mp3 | 25% death_alt.mp3 | 10% death_rare.mp3 */
  function playDeathSound() {
    const r = Math.random();
    let src;
    if (r < 0.10)      src = 'assets/sounds/death_rare.mp3';
    else if (r < 0.35) src = 'assets/sounds/death_alt.mp3';
    else               src = 'assets/sounds/death.mp3';
    try {
      const a = new Audio(src);
      a.volume = 1;
      a.play().catch(() => {
        // fallback → son principal
        const b = new Audio('assets/sounds/death.mp3');
  ...
  function triggerDeathEffect() {
    ...
    // Texte MOGGED — identique à la v7
  ...
  function beginGame() {
    ...
    obstacles = []; birds = []; particles = []; frame = 0; score = 0; speed = 5.5;
  ...
  /* ── Spawn oiseau — jamais dans la zone de clearance obligatoire d'un pipe ──
     On calcule la trajectoire maximale du saut au moment du spawn pour s'assurer
     qu'il existe au moins un couloir dégagé entre sol et plafond du saut.
     Si l'oiseau tomberait dans la zone [yMin_obstacle_clearance, GROUND_Y],
     on le repositionne en zone haute (au-dessus du saut max).
  ── */
  function spawnBird() {
    // Calcul apex du saut
    const t_apex  = -JUMP_V / GRAVITY;
    const y_apex  = (GROUND_Y - PLAYER_H) + JUMP_V * t_apex + 0.5 * GRAVITY * t_apex * t_apex;
    const jumpTop = y_apex - PLAYER_H;          // y le plus haut que peut atteindre le joueur
    const BIRD_H  = 78;
    const BIRD_W  = 78;

    // Est-ce qu'une pipe arrive dans les 380px ?
    ...
    if (!pipeIncoming) {
      // Pas de pipe proche : oiseau n'importe où, le joueur gère seul
      if (roll < 0.40) {
        // Zone haute : au-dessus du saut max (joueur peut passer dessous sans sauter)
        y = 15 + Math.random() * Math.max(5, jumpTop - 30);
      } else if (roll < 0.75) {
        // Zone basse : près du sol (joueur doit sauter)
        y = GROUND_Y - 90 - Math.random() * 80;
      } else {
        // Zone mi-haute : dans la trajectoire du saut, mais pas impossible
        y = jumpTop + 10 + Math.random() * 60;
      }
    } else {
      // Pipe proche : oiseau uniquement en zone haute (pas de saut forcé double)
  ...
  function drawGround() {
    ctx.save();
    // Translate le pattern pour qu'il commence exactement à GROUND_Y
    ctx.translate(0, GROUND_Y);
    ctx.fillStyle = woodPattern;
    ctx.fillRect(0, 0, W, WOOD_H);
    ctx.restore();
    // Liseré sombre de séparation
  ...
  /* ══════════════════════════════════════════
     BOOST UI — icône(s) sur la barre bois
  ══════════════════════════════════════════ */
  ...
  function updateBoostUI() {
    ...
    if (!bar) {
      ...
      bar.style.cssText = [
        ...
        'left:0',
        'width:100%',
        'height:55px',           // = WOOD_H, par-dessus la barre bois
        'display:flex',
        'align-items:center',
        'gap:6px',
        'padding-left:12px',
  ...
  function activateBoost() {
    if (!boostActive && boostCharges > 0 && state === 'running') {
      boostCharges--;
      boostActive = true;
  ...
  function tickBoost() {
    // Octroyer une charge tous les 200 pts (max stockage illimité jusqu'à mort)
  ...
  function drawPlayerWithBoost() {
    const cx = PL.x + PLAYER_W / 2, cy = PL.y + PLAYER_H / 2;
    ctx.save(); ctx.translate(cx, cy); ctx.rotate(PL.angle);
    const img = boostActive ? boostImg : playerImg;
    if (img.complete && img.naturalWidth > 0) {
      if (boostActive) {
        // Halo doré pendant l'invincibilité
  ...
  function loop() {
    ...
    // Oiseaux fréquents mais pas impossibles
