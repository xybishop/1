...
(function () {
  ...
  let lastTimestamp  = 0;
  let scoreAccum     = 0; // accumulateur ms pour le score (remplace frame%6)
  let spawnAccum     = 0; // accumulateur ms pour les spawns
  ...
  function beginGame() {
    ...
    state = 'running';
    lastTimestamp = 0; scoreAccum = 0; spawnAccum = 0;
    loop();
  }
  ...
  function gameOver() {
    ...
    muteGameBg();
    removeSectionBg();
    removeGameBg(); // immédiat — évite le son en double si on respawn vite
    document.querySelector('.game-wrap')?.classList.remove('playing');
    canRespawn = false; // bloqué dès la mort
    triggerDeathEffect(); // tremblement 1100ms
    ...
    deathTimeoutId = setTimeout(() => {
      ...
      // Autorise le respawn 500ms après la fin du tremblement (1100ms)
      setTimeout(() => { canRespawn = true; }, 500);
    }, 1100);
  }
  ...
  function loop(timestamp) {
    ...
    // Delta-time : normalise la vitesse quelle que soit la fréquence d'écran
    // dt = 1.0 à 60fps, 0.5 à 30fps, ~2.4 à 144fps → tout est ramené à 60fps
    if (!lastTimestamp) lastTimestamp = timestamp;
    const dt = Math.min((timestamp - lastTimestamp) / 16.667, 3); // cap à 3 pour éviter les sauts si l'onglet était en arrière-plan
    ...
    // Score : +1 toutes les 100ms (= frame%6 à 60fps)
    scoreAccum += dt * 16.667;
    if (scoreAccum >= 100) {
      scoreAccum -= 100;
      score++;
      speed = 5.5 + Math.floor(score/300) * 0.4;
    }

    // Spawns : basés sur le temps, pas sur le numéro de frame
    spawnAccum += dt * 16.667;
    const obsInterval = Math.max(900, 2200 - score * 1.3); // ms entre pipes
    if (spawnAccum >= obsInterval) {
      spawnAccum -= obsInterval;
      spawnObstacle();
      if (Math.random() < 0.75) spawnBird();
    }
    if (spawnAccum >= obsInterval * 0.5 && Math.random() < 0.007) spawnBird(); // oiseaux bonus aléatoires
