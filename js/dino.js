/* ══════════════════════════════════════════════════════════════
   ...
   • Sol bois, MOGGED, oiseaux, sons (65/25/10%)
   • BOOST : M en jeu = consomme 1 charge, -50 score, 5s invincibilité
...
(function () {
  ...
  const BOOST_DURATION  = 300; // 5s @ 60fps
  const VIDEO_START_T   = 85;  // secondes (1:25)
  ...
  /* ── Sons pré-chargés (buffer au load, pas au clic) ── */
  ...
  /* ══════════════════════════════════════════
     VIDÉOS — créées/détruites proprement, seek forcé sans attendre loadedmetadata
  ══════════════════════════════════════════ */
  ...
  function seekAndPlay(v, t) {
    // On seek dès que possible, on ne bloque pas sur loadedmetadata
    if (v.readyState >= 1) {
      v.currentTime = t;
      v.play().catch(() => {});
    } else {
      ...
      // Filet : si loadedmetadata tarde, on force quand même après 400ms
      setTimeout(() => {
        if (v.paused) {
  ...
  /* ══════════════════════════════════════════
     INPUTS — un seul listener sur window, pas de doublons
  ══════════════════════════════════════════ */
  ...
  window.addEventListener('keydown', function(e) {
    ...
    if (e.code === 'KeyM') {
      e.preventDefault();
      if (state === 'running') activateBoost();
    }
  });
  ...
  function beginGame() {
    ...
    // Détruire proprement les vidéos existantes AVANT d'en créer de nouvelles
    removeSectionBg();
    removeGameBg();
    removeBoostUI();
    ...
    // Créer les vidéos fraîches — elles seekent à VIDEO_START_T automatiquement
    createSectionBg();
    createGameBg();
  ...
  function drawObstacle(o) {
    for (let i = 0; i < o.count; i++) {
      ...
      if (pipeImg.complete && pipeImg.naturalWidth > 0) {
        ...
        ctx.fillStyle = '#b5271f'; ctx.fillRect(ox, oy, OBS_W, o.h);
      }
    }
  }
