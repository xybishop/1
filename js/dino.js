...
(function () {
  ...
  let lastLogicTime  = 0; // throttle 60fps — fix écrans haute fréquence
  ...
  function loop(ts) {
    ...
    // Throttle à 60fps : sur 144Hz on saute les frames en trop
    // La physique et toutes les valeurs restent identiques à 60fps
    if (!lastLogicTime) lastLogicTime = ts;
    const elapsed = ts - lastLogicTime;
    if (elapsed < 14) { // < ~14ms = trop tôt, on attend (16.67ms = 60fps)
